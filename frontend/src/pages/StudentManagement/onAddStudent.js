...
const StudentList = () => {
  ...
  const addStudent = async (student) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/students`, student);
  ...
  const updateStudent = async (updatedStudent) => {
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/api/students/${updatedStudent._id}`, updatedStudent);
      ...
      {filteredStudents.length === 0 ? (
        ...
        <table className="min-w-full bg-white text-sm shadow-md rounded-lg overflow-hidden mx-auto max-w-4xl">
          <thead className="bg-indigo-600 text-white">
            <tr>
              <th className="py-3 px-8 text-left">Nom et Prénom</th>
          ...
          <tbody>
            {filteredStudents.map((student) => (
              <tr key={student._id} className="border-b hover:bg-indigo-50">
                <td className="py-3 px-8">{student.name}</td>
