  ...
  useEffect(() => {
    if (searchQuery === '') {
      ...
      const filtered = teachers.filter(teacher => {
        ...
        return fullName.includes(searchQuery.toLowerCase()) || teacher.email.toLowerCase().includes(searchQuery.toLowerCase());
      });
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <table className="min-w-full bg-white text-sm shadow-md rounded-lg overflow-hidden mx-auto">
        <thead className="bg-gradient-to-r from-indigo-600 to-indigo-400 p-4 rounded-lg shadow-md max-w-3xl mx-auto mt-6">
          <tr>
            ...
            <th className="py-3 px-4 text-white text-left">Date du prochain paiement</th>
        ...
        <tbody>
          {filteredTeachers.map(teacher => (
            <tr key={teacher._id} className="border-b hover:bg-blue-50">
              ...
              <td className="py-3 px-4">{teacher.paymentDate}</td>
              <td className="py-3 px-4">{teacher.nextPaymentDate}</td>
